...
// Define an interface to describe the structure of cvInfo
interface CVInfo {
    q1: string; // Full name
    q2: string; // Contact number
    q3: string; // Age
    q4: string; // Email
}
...
if (userInput === 'Yes' || userInput === 'yes' || userInput === 'YES' || userInput === 'Y' || userInput === 'y'){
    let cvInfo : CVInfo = await inquirer.prompt([
        {  
            name: 'q1',
            type:'input',
    ...
    const academicQualifications: { degree: any; university: any; graduationYear: any; }[] = []; // Array to store multiple qualifications
    while (true) {
